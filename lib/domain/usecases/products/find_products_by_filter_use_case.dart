...
class FindProductsByFilterUseCaseImpl implements FindProductsByFilterUseCase {
  ...
  FindProductsByFilterUseCaseImpl(this.productRepository);
  ...
  Future<ProductsByFilterResult> execute(ProductsByFilterParams params) async {
    try {
      ...
      if (products != null && products.isNotEmpty) {
        ...
        products.forEach((product) => product.selectableAttributes != null
            ? result.addAll({
                for (var attribute in product.selectableAttributes)
                  attribute: []
              })
            : {});
